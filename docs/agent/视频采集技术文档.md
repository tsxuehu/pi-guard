...
| 数据结构 | 字段 | 说明 |
|---|---|---|
| `VideoFrame` | `seq: uint64_t` | 全局递增帧序号 |
...
| 启动 | `void start()` | 初始化 V4L2 并启动采集线程，失败抛 `runtime_error` | ❌ 不可并发 |
| 停止 | `void stop()` | 停止采集并释放 V4L2 资源 | ❌ 不可并发 |
...
视频采集和音频采集、编码器采用**同一套多消费者设计模式**。详见 [`多消费者技术方案.md`](./多消费者技术方案.md)，本节只摘录视频相关的关键信息。

### 5.1 核心机制

```mermaid
graph TD
    V4L2["V4L2 设备<br/>VIDIOC_DQBUF"] --> PT["cap_thread_<br/>(采集线程)"]
    PT -->|"入队 + notify_all()"| Q["queue_<br/>deque&lt;QueuedFrame&gt;<br/>pending_consumers"]
    Q -->|"wait_frame(id, last_seq)"| C1["消费者 A<br/>(编码器)"]
    Q -->|"wait_frame(id, last_seq)"| C2["消费者 B<br/>(移动侦测)"]
    Q -->|"wait_frame(id, last_seq)"| C3["消费者 C<br/>(预览)"]
```
...
2. **`wait_frame` 是阻塞的**：没有新帧时阻塞，直到有新数据或 `stop()`
3. **队列满时丢弃最旧帧**：`pop_front()` 移除最老数据
4. **新消费者从注册后的数据开始消费**：已有队列节点不会追加新消费者
5. **不要长时间持有帧**：帧数据是 mmap 映射的 V4L2 缓冲区，长时间持有会耗尽驱动缓冲
...
graph TD
    ...
    subgraph produce_loop
        ...
        FRAME --> ENQ["入队 queue_<br/>pending = consumers_"]
...
当最后一个引用释放时，`shared_ptr` 的删除器自动执行 `QBUF`，**不需要知道消费者什么时候用完**。

### 7.3 完整时序

```
T1: DQBUF idx=2 → 创建 frame(v4l2_ref 引用计数=1)
    queue_.push_back({frame, consumers_}) → shared_ptr 拷贝，引用计数=2

T2: 消费者 A wait → result 中持有 shared_ptr → 引用计数=3
    消费者 B wait → result 中持有 shared_ptr → 引用计数=4

T3: A 处理完释放 → 引用计数=3
    queue_ 中该节点被 erase（所有消费者取走）→ 引用计数=2

T4: B 处理完释放 → 引用计数=1

T5: 无其他引用 → shared_ptr 析构 → 删除器执行 QBUF(idx=2)
    缓冲区还回驱动，可再次被 DQBUF
```

> **关键保证**：即使消费者崩溃未释放帧，或队列溢出丢弃旧帧，`shared_ptr` 的引用计数机制确保 QBUF 最终一定会发生。
...
if (!start_ok_) {
    throw std::runtime_error("VideoCaptureProvider: failed to start");
}
...
**为什么 `stream_off()` 能唤醒 DQBUF？**

`VIDIOC_DQBUF` 在没有数据时会阻塞。调用 `VIDIOC_STREAMOFF` 后，阻塞的 `DQBUF` 会以 `EINVAL` 或 `EPIPE` 错误返回，采集线程检查 `running_` 后优雅退出。
