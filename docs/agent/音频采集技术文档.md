...
| 数据结构 | 字段 | 说明 |
|---|---|---|
| `AudioFrame` | `seq: uint64_t` | 全局递增帧序号（详见 [多消费者方案 4.3 节](./多消费者技术方案.md)） |
...
| 启动 | `void start()` | 初始化 ALSA 并启动采集线程，失败抛 `runtime_error` | ⚠️ 不应并发调用 |
| 停止 | `void stop()` | 停止采集并清理资源 | ⚠️ 不应并发调用 |
...
音频采集和视频采集、编码器采用**同一套多消费者设计模式**，核心机制（`seq` / `last_seq` 增量拉取、`pending_consumers` 追踪、队列溢出丢弃最老帧等）详见 [`多消费者技术方案.md`](./多消费者技术方案.md)。
...
音频采集使用 `start_mtx_ + start_cv_` 进行启动握手，机制与视频采集一致，详见 [`多消费者技术方案.md` 7.5 节](./多消费者技术方案.md)。

音频特有的启动细节：硬件初始化步骤为 `snd_pcm_open` → `snd_pcm_set_params` → `snd_pcm_start`（详见 6.2 节），任一步骤失败都会通过 `report_start_result(false)` 通知主线程，`start()` 抛出 `std::runtime_error`。

### 7.2 停止流程

停止机制与多消费者方案 7.7 节一致：`mark_stopped()` 设置 `running_ = false` + `notify_all()`，消费者被唤醒后返回空结果退出，生产者检查 `running_` 后退出循环，主线程 `join()` 等待所有线程结束。

音频特有的延迟：采集线程可能在 `snd_pcm_readi` 上阻塞（阻塞模式），`mark_stopped()` 只设置了标志，线程需要等待当前 `readi` 返回（最多 ~20ms，一个 ALSA 周期）后才能检查 `running_` 并退出。这是可接受的延迟。
