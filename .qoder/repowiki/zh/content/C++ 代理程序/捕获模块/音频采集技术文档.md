...
**更新** 音频帧存储在标准向量中，而非内存映射缓冲区，这与视频帧的内存映射机制形成重要区别。
...
**更新** 本节强调了音频和视频消费者管理的重要差异：

#### 音频消费者管理特点
- **状态跟踪变量**：使用 `active_consumers_` 集合跟踪活跃消费者
- **向量存储**：音频帧存储在标准向量中，便于批量处理
- **简单队列结构**：使用 `std::list<queued_audio>` 存储待分发的音频帧

#### 视频消费者管理特点
- **内存映射缓冲区**：使用 `std::vector<MMapBuffer>` 管理V4L2内存映射缓冲区
- **复杂队列结构**：使用 `std::deque<QueuedFrame>` 存储视频帧
- **缓冲区管理**：需要复杂的缓冲区分配、映射和回收机制

#### 关键差异对比

| 特性 | 音频 | 视频 |
|------|------|------|
| 帧存储 | std::vector<int16_t> | V4L2内存映射缓冲区 |
| 队列类型 | std::list | std::deque |
| 消费者集合 | std::set | std::unordered_set |
| 缓冲区管理 | 简单向量复制 | 复杂mmap管理 |
| 错误处理 | snd_pcm_recover | v4l2错误恢复 |

**章节来源**
- [audio_capture_provider.hpp:99](file://project/agent/src/modules/capture/audio/include/capture_audio/audio_capture_provider.hpp#L99)
- [video_capture_provider.hpp:97](file://project/agent/src/modules/capture/video/include/capture_video/video_capture_provider.hpp#L97)
...
**更新** 改进了启动和关闭程序的时序考虑说明：

#### 启动时序
1. **状态初始化**：设置 `running_=false` 和 `start_reported_=false`
2. **线程启动**：创建采集线程执行 `produce_loop()`
3. **同步等待**：等待启动结果报告，确保设备初始化完成
4. **错误处理**：启动失败时优雅退出并清理资源

#### 关闭时序
1. **标记停止**：调用 `mark_stopped()` 设置 `running_=false`
2. **线程同步**：等待采集线程安全退出
3. **资源清理**：关闭ALSA设备并清理队列
4. **消费者清理**：注销所有活跃消费者

#### 关键时序点
- **启动握手**：使用 `start_mtx_` 和 `start_cv_` 确保启动同步
- **停止通知**：使用 `state_cv_` 通知所有等待的消费者
- **资源回收**：确保在停止状态下不再产生新的音频帧

**章节来源**
- [audio_capture_provider.cpp:39-82](file://project/agent/src/modules/capture/audio/audio_capture_provider.cpp#L39-L82)
- [audio_capture_provider.cpp:161-168](file://project/agent/src/modules/capture/audio/audio_capture_provider.cpp#L161-L168)
...
**更新** 本次更新反映了代码重构中将 `consumers_` 变量重命名为 `active_consumers_` 的改进，以及对音频和视频消费者管理差异的深入分析。这些变更改善了音频采集模块的状态跟踪机制，使变量命名更加清晰地表达了其含义，提高了代码的可读性和维护性。同时，澄清了音频帧存储在向量中而非内存映射缓冲区的技术细节，为开发者提供了更准确的实现指导。