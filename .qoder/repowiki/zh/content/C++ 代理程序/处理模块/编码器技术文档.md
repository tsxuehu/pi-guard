...
- 明确同步机制使用单互斥锁+条件变量的设计
- 更新包队列容量默认值为300，明确溢出行为
- 改进并发警告表述，增强线程安全说明
...
**更新** 本次更新反映了编码器模块中多消费者状态管理的重构，将 `consumers_` 变量重命名为 `active_consumers_`，优化了状态管理的实现细节，提高了代码的可读性和维护性。同时明确了同步机制使用单互斥锁+条件变量的设计，包队列容量默认值为300，溢出时丢弃最老的包，并改进了并发警告表述，增强了线程安全说明。